...
export const calculateCostOfCapital = (company) => {
  ...
  if (approach === CocApproaches.AVERAGE) {
    ...
    const totalEv = businesses.reduce((acc, cur) => (
  ...
  const coc = equityValue / totalCapital * coe +
...
export const calculateValueOfDebt = (company) => {
  ...
  const result = interestExpense * (1 - (1 + cod) ** (-averageMaturity)) 
                 / cod + bookDebt / (1 + cod) ** (averageMaturity)
...
const calculateLeveredBeta = (company) => {
  ...
  const result = unleveredBeta * (1 + (1 - taxRate) *
                 (valueOfDebt / valueOfEquity))
...
export const calculateErp = (company) => {
  ...
  if (erpApproach === ErpApproaches.COUNTRIES) {
    ...
    const result = countries.reduce(
      (acc, cur) =>
        (acc + cur.revenue / totalRevenue
             * marketData.countries[cur.country]['ERP']),
      0
    )
  ...
  const result = regions.reduce(
    (acc, cur) =>
      (acc + cur.revenue / totalRevenue * marketData.regions[cur.region]['ERP']),
    0
  )
...
export const calculateLongRevenueGrowthRate = (company) => {
  const rfr = calculateRiskFreeRateLong(company)
  return company.overrides.revenueGrowth ?
         company.revenueGrowth.long :
         rfr
}
...
export const calculateOperatingMargin = (company) => {
  ...
  const adjustedEbit = hasRdExpenses ? ebit + calculateEbitRdEffect(company) : ebit
 ...
 */
export const projectEbit = (company, projectedRevenue, projectedOperatingMargin) => {
...
export const projectTaxRate = (company) => {
  ...
  const taxRateLong = taxRateOverride ? company.taxRateEffectiveLong : marketData.taxRate
...
export const projectTaxExpense = (
  ...
  for (let i = 1; i < projectedEbit.length; ++i) {
    ...
    const taxExpense = nol > projectedEbit[i] ? 
                       0 :
...
export const projectReinvestmentExpense = (
  ...
  result.push(
    projectedRevenueGrowthRate.at(-1) / 100 /
    (rocLong / 100) *
    (projectedEbit.at(-1) - 
    projectedTaxExpense.at(-1))
  )
...
export const projectRoic = (
  ...
  let firstResult = (projectedEbit[0] - projectedTaxExpenses[0]) /
                projectedInvestedCapital[0] * 100
  ...
  for (let i = 1; i < projectedEbit.length - 1; ++i) {
    let entry = (projectedEbit[i] - projectedTaxExpenses[i]) /
                 projectedInvestedCapital[i-1] * 100
...
export const calculateTerminalValue = (
  ...
  const result = projectedFcff.at(-1) / 
                 (projectedCoc.at(-1) - projectedRevenueGrowthRate.at(-1)) * 100
 ...
 */
export const calculateValueOfOptions = (company, rfr = calculateRiskFreeRateLong(company) / 100) => {
  ...
  const d1 = (Math.log(stockPrice / strike) + (rfr + iv ** 2) * maturity) /
  ...
  const result = stockPrice * cdf(d1, 0.0, 1.0) - strike *
                 Math.exp(-rfr * maturity) * cdf(d2, 0.0, 1.0)
