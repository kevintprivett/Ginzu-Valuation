  ...
  <TableRow>
    ...
    {data.map((item, idx) => (
      isNaN(item) ? 
        ...
        <SmallTableCell key={`${label}${idx}`}>{`${prefix}${wrapper(item)}${suffix}`}</SmallTableCell>
    ))}
  </TableRow>
)
...
const ValuationBarChart = (
  ...
  const failureValue = failureChance * (failureProceeds - (pvTerminalValue + pvCf)) / 100
...
const ValuationOutputPanel = () => {
  ...
  const projectedEbit = projectEbit(company, projectedRevenue, projectedOperatingMargin)
  ...
  const pvTerminalValue = calculatePvTerminalValue(terminalValue, projectedDiscountFactor)
  const pvCf = calculatePvCf(projectedPvFcff)
  const sumOfPv = pvTerminalValue + pvCf
  const failureProceeds = sumOfPv / 2
  const valueOfOperatingAssets = sumOfPv * (1 - company.failureChance / 100) +
  ...
  const valuePerShare = company.shares === 0 ? 0 : valueOfEquityCommon / company.shares
  const priceValue = valuePerShare === 0 ? 0 : company.stockPrice / valuePerShare * 100

  const decimalWrapper = (input) => (input.toFixed(2))
  const currencyWrapper = (input) => (input.toLocaleString('en-US', {
