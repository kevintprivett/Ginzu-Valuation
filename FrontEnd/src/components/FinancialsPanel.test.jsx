...
describe('FinancialsPanel', async () => {
  ...
  describe('Verify search links', () => {
    test('Link appears when ticker and name is present', async () => {
      const linkBeforeYahoo = screen.queryByText(
        /Yahoo Finance Search/
      )

      const linkBeforeEdgar = screen.queryByText(
      ...
      const linkAfterYahoo = await screen.findByText(
        /Yahoo Finance Search/
      )
      ...
      const linkAfterEdgar = await screen.findByText(
        /EDGAR Filings Search/
      )
