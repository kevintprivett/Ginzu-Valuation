    ...
    >
      ...
      >
        <Typography
          variant='body1'
        >
          Book Value of Debt: ${company.debt['Q10'].toLocaleString('US-en', {
        ...
        <Typography
          variant='body1'
        >
          Estimated Value of Debt: ${calculateValueOfDebt(company).toLocaleString('US-en', {
            maximumFractionDigits: 2
          })}MM
        </Typography>
      </Stack>
      ...
      >
        ...
        >
          {Object.keys(CodApproaches).map(key => {
            return (
              <MenuItem value={CodApproaches[key]}>{CodApproaches[key]}</MenuItem>
            )}
          )}
        </Select>
      </Stack>
      {company.codApproach === CodApproaches.DIRECT && <CostOfDebtDirect />}
      {company.codApproach === CodApproaches.SYNTHETIC && <CostOfDebtSynthetic />}
