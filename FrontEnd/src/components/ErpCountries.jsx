    ...
    <>
      ...
      >
        <Select
          ...
          onChange={(event) => handleObjectArrayChange('countries', index, 'country', event.target.value)}
          MenuProps={{
            PaperProps: {
              sx: {
                maxHeight: 400
        ...
        >
          {Object.keys(Countries).map(key => {
            return (
              <MenuItem value={Countries[key]}>{Countries[key]}</MenuItem>
            )}
          )}
        </Select>
      </Grid >
      <Grid size={2}>
        <NumericFormat
          ...
          onValueChange={(values) => {
            handleObjectArrayChange('countries', index, 'revenue', values.floatValue)
          }}
