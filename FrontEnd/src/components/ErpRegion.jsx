    ...
    <>
      ...
      >
        <Select
          ...
          onChange={(event) => handleObjectArrayChange('regions', index, 'region', event.target.value)}
          MenuProps={{
            PaperProps: {
              sx: {
                maxHeight: 400
        ...
        >
          {Object.keys(Regions).map(key => {
            return (
              <MenuItem value={Regions[key]}>{Regions[key]}</MenuItem>
            )}
          )}
        </Select>
      </Grid >
      <Grid size={2}>
        <NumericFormat
          ...
          onValueChange={(values) => {
            handleObjectArrayChange('regions', index, 'revenue', values.floatValue)
          }}
